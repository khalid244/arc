...
func TestVariantPath_1d(t *testing.T) {
	...
	want := "precalc/table=default.events/tier=1d/year=2026/month=05/day=15/by_site/abc.parquet"
	if got != want {
...
func TestVariantPath_1w(t *testing.T) {
	...
	want := "precalc/table=default.events/tier=1w/year=2026/week=20/sketch/xyz.parquet"
	if got != want {
		t.Errorf("got %q want %q", got, want)
	}
}
...
func TestVariantPath_1mo(t *testing.T) {
	...
	want := "precalc/table=default.events/tier=1mo/year=2026/month=05/all/m1.parquet"
	if got != want {
		t.Errorf("got %q want %q", got, want)
	}
}
