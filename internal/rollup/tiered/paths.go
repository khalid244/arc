...
// in the _arc/rollup/ hive-partitioned layout. Schema:
//
//   _arc/rollup/table=<T>/tier=<G>/year=<Y>/[partition]/<variant>/<file_id>.parquet
//
...
func VariantPath(table string, tier Tier, variant string, bucket time.Time, fileID string) string {
	prefix := fmt.Sprintf("_arc/rollup/table=%s/tier=%s/year=%04d", table, tier, bucket.Year())
	switch tier {
	case Tier1h, Tier1d:
...
func TmpPath(table, fileID string) string {
	return fmt.Sprintf("_arc/rollup/table=%s/tmp/%s.parquet", table, fileID)
}

// ManifestPath returns the manifest.json key for a table.
func ManifestPath(table string) string {
	return fmt.Sprintf("_arc/rollup/table=%s/manifest.json", table)
}

// SpecPath returns the spec.json key for a table.
func SpecPath(table string) string {
	return fmt.Sprintf("_arc/rollup/table=%s/spec.json", table)
}
